...
export default function TestimonialsSection() {
  return (
    <section id="testimonials" className="section-padding bg-ink-light">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal>
          <p className="eyebrow mb-4 text-center">Stories</p>
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-light text-cream tracking-tight text-center text-balance mb-16">
            Words from Those Who've Experienced It
          </h2>
        </ScrollReveal>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <ScrollReveal key={t.name} delay={i * 0.1}>
              <div className="relative bg-ink-surface/40 border border-gold/10 p-8 md:p-10">
                {/* Top shimmer line */}
                <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gold/40 to-transparent" />

                {/* Large quote mark */}
                <span className="font-display text-6xl text-gold/10 leading-none select-none">"</span>

                <p className="font-display text-base md:text-lg italic text-cream/70 leading-relaxed mt-2 mb-8">
                  {t.quote}
                </p>

                <p className="font-body text-xs tracking-[0.15em] uppercase text-gold">
                  {t.name}
                </p>
